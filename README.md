...
- **Minimum order-count threshold:** The Top 10 Products table (Overview page) applies a minimum order-volume filter before ranking, to prevent low-volume products (e.g., a single-order item with a 50% cancellation rate) from producing misleading, statistically unreliable percentages.
...
**Top 10 Products: Revenue & Order Risk:**
A table (Overview page) ranking the top 10 merchandise products by net revenue, showing total orders, cancelled orders, and cancellation rate side by side — filtered to merchandise only and to products meeting a minimum order-volume threshold, so every rate shown reflects a meaningful sample size.
...
- Average order value strengthened toward the end of the period, reaching its highest levels in September–November 2011 after several months of volatility.
